...
(function (LexAI) {
  ...
  async function generateTemplate() {
    ...
    def.fields.forEach(f => {
      ...
      values[f.id] = raw ? LexAI.Security.sanitizeTemplateInput(LexAI.Security.sanitizePromptInput(raw)) : `[${f.label}]`;
    });
    ...
    const rl = LexAI.Security.checkRateLimit("template");
    if (!rl.allowed) { LexAI.UI.showToast("Too many generation requests. Please wait a moment.", "error"); return; }
...
Format as a clean, professional document. Use proper legal formatting.`;
    ...
    try {
      ...
      templateOutput.innerHTML = LexAI.Security.escapeHtml(aiText);
      ...
      $("copy-template-btn")?.addEventListener("click", () => {
        navigator.clipboard.writeText(aiText).then(() => LexAI.UI.showToast("Template copied!", "success"));
      });
      $("download-template-btn")?.addEventListener("click", () => {
        LexAI.Utils.downloadText(aiText, `${activeTemplateType}-template.txt`);
      });

      templateOutput.scrollIntoView({ behavior: "smooth", block: "nearest" });
      LexAI.UI.showToast("Template generated!", "success");
    } catch (err) {
      LexAI.UI.showToast("Generation failed: " + err.message, "error");
    } finally {
      isLoading = false;
